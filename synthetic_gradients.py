    ...
    def __init__(self, units, inputs, name, out=False, sg=False):
        ...
        with tf.variable_scope(self.name):
            if sg:
                ...
                self.output = Dense(units, kernel_initializer=tf.zeros_initializer())(
                    inputs_c
                )
            else:
                self.output = Dense(units)(inputs)
                if out:
                    self.output = Activation("relu")(BatchNormalization()(self.output))
        self.layer_vars = tf.get_collection(
...
class ModelSG(object):
    ...
    def __init__(self, sess):
        self.sess = sess
        self.dataset = tf.contrib.learn.datasets.mnist.read_data_sets(
            "data/", one_hot=True
        )
    ...
    def train_layer_n(self, h_m, h_n, d_hat_m, class_loss, next_l, d_n=None, p=True):
        ...
        if p:
            h_n = self.layers[h_n].output
        with tf.variable_scope(self.layers[h_m].name):
            layer_grads = tf.gradients(
                h_n, [self.layers[h_m].output] + self.layers[next_l].layer_vars, d_n
            )
            layer_gv = list(zip(layer_grads[1:], self.layers[next_l].layer_vars))
            layer_opt = tf.train.AdamOptimizer(
                learning_rate=self.learning_rate
            ).apply_gradients(layer_gv)
        with tf.variable_scope(self.synth_layers[d_hat_m].name):
            d_m = layer_grads[0]
            sg_loss = tf.divide(
                tf.losses.mean_squared_error(self.synth_layers[d_hat_m].output, d_m),
                class_loss,
            )
            sg_opt = tf.train.AdamOptimizer(learning_rate=self.learning_rate).minimize(
                sg_loss, var_list=self.synth_layers[d_hat_m].layer_vars
            )
    ...
    def prepare_training(self, learning_rate):
        ...
        self.reduce_lr = tf.assign(
            self.learning_rate, self.learning_rate / self.lr_div, name="lr_decrease"
        )

        self.pred_loss = tf.losses.softmax_cross_entropy(
            onehot_labels=self.inputs[1],
            logits=self.layers[3].output,
            scope="prediction_loss",
        )

        layer4_opt, sg4_opt = self.train_layer_n(
            2, self.pred_loss, 2, self.pred_loss, 3, p=False
        )
        ...
        layer1_opt = tf.train.AdamOptimizer(learning_rate=self.learning_rate).minimize(
            self.layers[0].output,
            var_list=self.layers[0].layer_vars,
            grad_loss=self.synth_layers[0].output,
        )

        self.decoupled_training = [
            [layer1_opt],
            [layer2_opt, sg2_opt],
            [layer3_opt, sg3_opt],
            [layer4_opt, sg4_opt],
            ...
            for i in tqdm(range(1, iterations + 1)):
                ...
                for d in self.decoupled_training:
                    if random.random() <= update_prob:
                        self.sess.run(d, feed_dict={X: data, Y: target})
    ...
    def test(self, batch_size):
        ...
        correct_preds = tf.equal(
            tf.argmax(preds, 1), tf.argmax(Y, 1), name="correct_predictions"
        )
        accuracy = (
            tf.reduce_sum(
                tf.cast(correct_preds, tf.float32), name="correct_prediction_count"
            )
            / batch_size
        )
        with self.sess.as_default():
            ...
            for _ in range(n_batches):
                ...
                batch_accuracy, batch_loss = self.sess.run(
                    [accuracy, self.pred_loss], feed_dict={X: Xb, Y: Yb}
                )
